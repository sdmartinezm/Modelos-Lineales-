...
if(nzchar(system.file(package = "xaringanBuilder")) == FALSE) {remotes::install_github("jhelvy/xaringanBuilder", upgrade = "always")}
...
<div class="container">
    <div class="container__image">
        <img src="assets/images/unal.svg" alt="logo universidad">
    </div>
    <div class="container__footer">
        <span>PROYECTO <span class="bold">CULTURAL, CIENTÍFICO Y COLECTIVO</span> DE NACIÓN</span>
    </div>
</div>
</body>
...
A Bernoulli event is one for which the probability the event occurs is p and the probability the event does not occur is 1-p; i.e., the event is has two possible outcomes (usually viewed as success or failure) occurring with
probability p and 1-p, respectively.

A Bernoulli distribution is the pair of probabilities of a Bernoulli event, which is too simple to be interesting. However, it is implicitly used in “yesno” decision processes where the choice occurs with the same probability
from trial to trial (e.g., the customer chooses to go down aisle 1 with probability p).
...
- Integer values: you do not count $7.56$ individuals;
  - You can say that *in average*, you expect $7.46$ individuals in a given situation. But you measure something *discrete*.
